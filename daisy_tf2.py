...
class DaisyTF2(object):
    """ Computes dense DAISY [1] descriptors from a batch of images. It uses
    Tensorflow 2.0 to perform operations on GPU (if available) to significantly
    ...
    def extract_descriptor(self,
        ...
        """
        images = tf.expand_dims(tf.stack(images, axis=0), 1)
    ...
    def _compute_ring_histograms(self,
        ...
        hist_smooth = tf.nn.conv2d(
            hist_smooth, self.gauss_dx, strides=1, padding='VALID',
            data_format='NCHW')
        hist_smooth = tf.pad(
            hist_smooth, [[0, 0], [0, 0], [radius, radius], [0, 0]], 'REFLECT')
        hist_smooth = tf.nn.conv2d(
            hist_smooth, self.gauss_dy, strides=1, padding='VALID',
            data_format='NCHW')
